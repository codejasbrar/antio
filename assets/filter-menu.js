function deferFilterjQuery() {
    if (window.jQuery) {
        ...
        $(function(){
          ...
          $.getScript("//cdn.shopify.com/s/files/1/0073/8102/4820/t/46/assets/jquery.pjax.js?31419").done(function(script, textStatus) {
            $body = $("body");
            $(document).pjax('#main .filter-group a, #main .pagination a', '#main', { fragment: '#main', timeout: 10000, scrollTo: false, push: true });
            $(document).on('pjax:start', function(){  $body.addClass("pt-ajax-loading"); $('#main').fadeTo('slow', 0.8); })
            $(document).on('pjax:end', function(){ $body.removeClass("pt-ajax-loading"); $('#main').fadeTo('fast', 1); pt_on_load(); ; });
          });
